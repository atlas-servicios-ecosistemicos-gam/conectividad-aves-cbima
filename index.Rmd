...
pal_bosque_bripario_IF_colors <- 
  unique(pal_bosque_bripario_IF(sort(sf_bosque_bripario$IF_)))
```

Row
-----------------------------------------------------------------------

### Servicios web: [Teselas esenciales e importantes](https://services9.arcgis.com/RrvMEynxDB8hycVO/arcgis/rest/services/gam_conectividad_parches_esenciales_importantes_bripario/FeatureServer)
```{r}
leaflet() %>%
  #fitBounds(
  ...
  addLegend(
    position = "bottomright",
    colors = pal_bosque_bripario_IF_colors,
    labels = c("1", "2", "3", "4", "5"),
    group = "Teselas importantes",
    title = "Importancia"
  ) %>%    
  addLayersControl(
    ...
    overlayGroups = c("Límite del CBIMA", "Teselas esenciales", "Teselas importantes"),
